...
class OpenSCADObject:
    ...
    def __init__(self, name: str, params: dict, bounding_box=None, origin=(0, 0, 0)):
        self.name = name
        self.params = params
        ...
        self.bounding_box: Optional[objects.P3] = bounding_box
        self.origin: objects.P3 = origin

    # Custom stuff
    def with_bounding_box(self, bounding_box: objects.P3) -> OpenSCADObject:
        self.bounding_box = bounding_box
        return self

    def place_relative_to(self, other: OpenSCADObject,
                          other_point_relative: objects.P3,
                          self_point_relative: objects.P3 = (0, 0, 0)) -> OpenSCADObject:

        assert other.bounding_box is not None \
               and self.bounding_box is not None, "Both elements must have a bounding_box. You can " \
                                                  "add a bounding box to an object using .with_bounding_box()"

        other_point_absolute = tuple(other.origin[idx]+other.bounding_box[idx]*other_point_relative[idx] for idx in range(3))
        self_point_absolute = tuple(self.origin[idx]+self.bounding_box[idx]*self_point_relative[idx] for idx in range(3))
        translation = tuple(other_point_absolute[i] - self_point_absolute[i] for i in range(3))
        return objects.translate(translation)(self)

    def center_behind(self, other, self_point_relative: objects.P3 = (0, 0, 0)):
        return self.place_relative_to(other, (.5, 1, .5), self_point_relative)

    def translate(self, translation: objects.P3) -> OpenSCADObject:
        return objects.translate(translation)(self)

    def move_up(self, z) -> OpenSCADObject:
        return self.translate((0, 0, z))

    def move_down(self, z) -> OpenSCADObject:
        return self.translate((0, 0, -z))

    def move_right(self, x) -> OpenSCADObject:
        return self.translate((x, 0, 0))

    def move_left(self, x) -> OpenSCADObject:
        return self.translate((-x, 0, 0))

    def move_forward(self, y) -> OpenSCADObject:
        return self.translate((0, y, 0))

    def move_back(self, y) -> OpenSCADObject:
        return self.translate((0, -y, 0))

    def get_extent(self) -> objects.P3:
        assert self.bounding_box is not None, f"Tried to get extent for objects {self.name}, but no bounding_box was set"
        return tuple(self.origin[i] + self.bounding_box[i] for i in range(3))

    @staticmethod
    def get_combined_origin(objects: Sequence[OpenSCADObject]) -> objects.P3:
        all_origin_x, all_origin_y, all_origin_z = zip(*[o.origin for o in objects])
        return min(all_origin_x), min(all_origin_y), min(all_origin_z)

    @staticmethod
    def get_combined_extent(objects: Sequence[OpenSCADObject]) -> objects.P3:
        """
        Calculates the maximum extent(=the outermost x, y, and z-value) of all objects in teh sequence
        :param objects:
        :return:
        """
        all_extent_x, all_extent_y, all_extent_z = zip(*[o.get_extent() for o in objects])
        return max(all_extent_x), max(all_extent_y), max(all_extent_z)

    @staticmethod
    def get_combined_bounding_box(objects: Sequence[OpenSCADObject]) -> objects.P3:
        """
        Bounding box of the union of all objects in the sequence
        :param objects:
        :return:
        """
        for o in objects:
            assert o.bounding_box is not None, f"Tried to access bounding_box of {o.name}, but none was set."

        origin = OpenSCADObject.get_combined_origin(objects)
        extent = OpenSCADObject.get_combined_extent(objects)
        return tuple(extent[i]-origin[i] for i in range(3))


    def rotate(self, a: Union[float, objects.Vec3] = None, v: objects.Vec3 = None):
