...
from typing import Dict, Optional, Sequence, Tuple, Union, List, NamedTuple
...
RELATIVE_ORIGIN = (0,0,0)
RELATIVE_CENTER = (.5,.5,.5)
...
class cube(OpenSCADObject):
    ...
    def __init__(self, size: ScadSize = None, center: bool = None, origin=(0, 0, 0)) -> None:
        super().__init__('cube',
                         {'size': size, 'center': center}, bounding_box=size, origin=origin)
...
class cylinder(OpenSCADObject):
    ...
    def __init__(self, r: float = None, h: float = None, r1: float = None, r2: float = None,
        ...
        radii = [radius for radius in [r, r1, r2] if radius is not  None]
        radii += [diameter/2 for diameter in [d, d1, d2] if diameter is not None]
        max_radius = max(radii)

        super().__init__('cylinder',
                         {'r': r, 'h': h, 'r1': r1, 'r2': r2, 'd': d,
                          'd1': d1, 'd2': d2, 'center': center,
                          'segments': segments}, bounding_box=(max_radius*2, max_radius*2, h), origin=(-max_radius, -max_radius, 0))
...
class union(OpenSCADObject):
    ...
    def __call__(self, *args: "OpenSCADObject") -> "OpenSCADObject":
        if args != ():
            child = args
            is_sequence = isinstance(child, collections.Sequence)
            original_bounding_box = OpenSCADObject.get_combined_bounding_box(child) if is_sequence else child.bounding_box
            original_origin = OpenSCADObject.get_combined_origin(child) if is_sequence else child.origin

            self.origin = original_origin
            self.bounding_box = original_bounding_box
...
class difference(OpenSCADObject):
    ...
    def __init__(self, calculate_bounding_box=True) -> None:
        self.calculate_bounding_box = calculate_bounding_box
        super().__init__('difference', {})

    def __call__(self, *args: "OpenSCADObject") -> "OpenSCADObject":
        child = args
        if isinstance(child, collections.Sequence) and len(child) > 1 and self.calculate_bounding_box:
            first = child[0]
            others = child[1:]
            all_partitions = get_all_partitions(others, [first])
            self.origin = OpenSCADObject.get_combined_origin(all_partitions)
            self.bounding_box = OpenSCADObject.get_combined_bounding_box(all_partitions)
...
class translate(OpenSCADObject):
    ...
    def __call__(self, *args: "OpenSCADObject") -> "OpenSCADObject":
        ...
        if child != ():
            ...
            self.origin = tuple(original_origin[i]+self.params['v'][i] for i in range(3))
            self.bounding_box = original_bounding_box
...
class rotate(OpenSCADObject):
    ...
    def __init__(self, a: Union[float, Vec3] = None, v: Vec3 = None, calculate_bounding_box=True) -> None:
        self.calculate_bounding_box = calculate_bounding_box
        self.a = a
        self.v = v
    ...
    def __call__(self, *args, **kwargs):
        if args[0].bounding_box is not None:
            bounding_box = args[0].bounding_box
            old_origin = args[0].origin

            if isinstance(self.a, collections.Sequence):
                x_angle, y_angle, z_angle = [np.deg2rad(self.a[i]) for i in range(3)]
            else:
                x_angle, y_angle, z_angle = np.deg2rad(self.a*self.v[0]), \
                                            np.deg2rad(self.a*self.v[1]), \
                                            np.deg2rad(self.a*self.v[2])
            ...
            vertex_roots = [(0, 0, 0), bounding_box]
            vertices = [(
                vertex_roots[i & 1][0],
                vertex_roots[i >> 1 & 1][1],
                vertex_roots[i >> 2 & 1][2]) for i in range(8)]

            rotation_matrix = np.matmul(rx, ry, rz)
            vertices_rotated = [rotation_matrix.dot(np.array(v)) for v in vertices]
            all_x, all_y, all_z = zip(*vertices_rotated)
            new_origin = min(all_x), min(all_y), min(all_z)
            new_extent = max(all_x), max(all_y), max(all_z)
            new_bounding_box = tuple(new_extent[i]-new_origin[i] for i in range(3))

            old_origin_rotated = rotation_matrix.dot(np.array(old_origin))
            self.bounding_box = new_bounding_box
            self.origin = tuple(old_origin_rotated[i]+new_origin[i] for i in range(3))
        else:
...
class color(OpenSCADObject):
    ...
    def __call__(self, *args, **kwargs):
        self.bounding_box = args[0].bounding_box
        self.origin = args[0].origin
        return super().__call__(*args)
